...
integer                                                  :: iil, iih, ijl, ijh  ! Cartesian area relatively to the entire domain
integer                                                  :: jp                  ! Process loop counter
...
!     tzbudiachro%ccomments(NLVL_MASK)        = DONE AFTER
  ...
  if ( tzfields(1)%ndimlist(6) == NMNHDIM_BUDGET_LES_MASK ) then
    ...
    if ( Trim( tpdiafile%cformat ) == 'NETCDF4' ) then
      ...
      do jp = 1, Size( hmasks )
        ...
        call Write_diachro( tzfile, tzbudiachro, [ tzfields(jp) ], zwork6(:,:,:,:,:,jp:jp) )
      end do
    end if
  else
    ...
    call Write_diachro( tpdiafile, tzbudiachro, tzfields, zwork6 )
  end if
...
integer                                              :: iil, iih, ijl, ijh ! cartesian area relatively to the
integer                                              :: iresp    ! return code
integer                                              :: ji
integer                                              :: jt       ! time counter
...
!            ----------------------
if ( iresp == 0 ) call Write_diachro( tpdiafile, tzbudiachro, [ tzfield ], zwork6 )
...
character(len=6)                                     :: ystring
integer                                              :: iresp    ! return code
integer                                              :: iil, iih, ijl, ijh !cartesian area relative to the entire domain
integer                                              :: ji
integer                                              :: jt       ! time counter
integer                                              :: jk       ! level counter
...
call Write_diachro( tpdiafile, tzbudiachro, [ tzfield ], zwork6 )
!
!* time average (physical units remain unchanged)
!
iresp = 0
