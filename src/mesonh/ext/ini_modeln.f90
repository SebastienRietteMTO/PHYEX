...
!!     the SET-REF call).
!!       - The prognostic variables are read in initial file (in READ_FIELD)
...
!!       - Then the initial file is closed by IO_File_close.
...
!!                   Nov.  29,2002 (JP Pinty) add ICE2, ICE4, ELEC
...
!  S. El Gdachi    06/2025: CAMS fields interpolation at each time step for CCN/IFN
!---------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
USE MODD_CH_M9_n,           only:NNONZEROTERMS, NEQ
...
USE MODD_PARAM_ECRAD_n,     ONLY: CGAS_MODEL_NAME
...
USE MODI_UPDATE_NSV
USE M_INIT_AER_ACLIB,  ONLY: INIT_AER_ACLIB
USE M_INIT_CHEM_ACLIB, ONLY: INIT_CHEM_ACLIB
USE M_INIT_AER_MODEL,  ONLY: INIT_AER_MODEL
USE MODD_ACLIB_n, ONLY: YPAER_ACLIB, YAEROUT_ACLIB, YAEROSOLS, YPCHEM_ACLIB
USE MODD_ACLIB,        ONLY: LUSECHEMTABLE
!
USE MODD_FIRE_n
IMPLICIT NONE
!
...
REAL, DIMENSION(:,:,:,:), POINTER :: DPTR_XLSSVM,DPTR_XLSSVS
!
...
IF (.NOT. LOCEAN) THEN
 IF (LFORCING) THEN ! Retrieve the number of time-dependent forcings.
  ...
  IF ( (IRESP /= 0) .OR. (NFRC <=0) ) THEN
    WRITE(ILUOUT,'(A/A)') &
     "INI_MODEL_n ERROR: you want to read forcing variables from initial file", &
 ...
 ! Modif PP for time evolving adv forcing
  IF ( L2D_ADV_FRC ) THEN ! Retrieve the number of time-dependent forcings.
    ...
    IF ( (IRESP /= 0) .OR. (NADVFRC <=0) ) THEN
      WRITE(ILUOUT,'(A/A)') &
      "INI_MODELn ERROR: you want to read forcing ADV variables from initial file", &
      "                   but no fields have been found by IO_Field_read"
    !callabortstop
      CALL PRINT_MSG(NVERB_FATAL,'GEN','INI_MODEL_n','')
    END IF
 ...
 IF ( L2D_REL_FRC ) THEN ! Retrieve the number of time-dependent forcings.
    ...
    IF ( (IRESP /= 0) .OR. (NRELFRC <=0) ) THEN
      WRITE(ILUOUT,'(A/A)') &
      "INI_MODELn ERROR: you want to read forcing REL variables from initial file", &
      "                   but no fields have been found by IO_Field_read"
    !callabortstop
      CALL PRINT_MSG(NVERB_FATAL,'GEN','INI_MODEL_n','')
    END IF
...
!$acc enter data copyin(XUT,XVT,XWT)
...
!
IF (CTURB /= 'NONE') THEN
  ALLOCATE(XTKET(IIU,IJU,IKU)) ; XTKET(:,:,:) = 0.0
  ALLOCATE(XRTKES(IIU,IJU,IKU)) ; XRTKES(:,:,:) = 0.0
  ALLOCATE(XRTKEMS(IIU,IJU,IKU)); XRTKEMS(:,:,:) = 0.0
  ALLOCATE(XWTHVMF(IIU,IJU,IKU)) ; XWTHVMF(:,:,:) = 0.0
  ALLOCATE(XWUMF(IIU,IJU,IKU)) ; XWUMF(:,:,:) = 0.0
  ALLOCATE(XWVMF(IIU,IJU,IKU)) ; XWVMF(:,:,:) = 0.0
  ALLOCATE(XDYP(IIU,IJU,IKU)) ; XDYP(:,:,:) = 0.0
  ALLOCATE(XTHP(IIU,IJU,IKU)) ; XTHP(:,:,:) = 0.0
  ALLOCATE(XTR(IIU,IJU,IKU)) ; XTR(:,:,:) = 0.0
  ALLOCATE(XDISS(IIU,IJU,IKU)) ; XDISS(:,:,:) = 0.0
  ALLOCATE(XLEM(IIU,IJU,IKU)) ; XLEM(:,:,:) = 0.0
ELSE
...
END IF
IF (CTOM == 'TM06') THEN
  ALLOCATE(XBL_DEPTH(IIU,IJU)) ; XBL_DEPTH(:,:) = 0.0
ELSE
  ALLOCATE(XBL_DEPTH(0,0))
END IF
IF (LRMC01) THEN
  ALLOCATE(XSBL_DEPTH(IIU,IJU)) ; XSBL_DEPTH(:,:) = 0.0
ELSE
...
!$acc enter data copyin(XPABSM,XPABST)
!
...
ALLOCATE(XSEA_UCU(IIU,IJU)) ;         XSEA_UCU(:,:) = 0.0
ALLOCATE(XSEA_VCU(IIU,IJU)) ;         XSEA_VCU(:,:) = 0.0
!
IF (LPASPOL) THEN
...
!$acc enter data create(XDIRCOSZW,XDIRCOSXW,XDIRCOSYW)
ALLOCATE(XCOSSLOPE(IIU,IJU))
ALLOCATE(XSINSLOPE(IIU,IJU))
!$acc enter data create(XCOSSLOPE,XSINSLOPE)
!
...
! in coupled O-A case, need different variables for ocean
   ALLOCATE(XRHODREFZO(IKU))
   ALLOCATE(XTHVREFZO(IKU))   
ELSE
...
ALLOCATE(XLSSVM(IIU,IJU,IKU,NSV)) ; XLSSVM = 0.0
!
!  lbc part
!
IF ( L1D) THEN                         ! 1D case
...
!
  ...
  ALLOCATE(XLSSVS(SIZE(XLSSVM,1),SIZE(XLSSVM,2),SIZE(XLSSVM,3),SIZE(XLSSVM,4)))
!
ELSE
!
...
IF (CRAD == 'ECRA') THEN
  IF (CGAS_MODEL_NAME == "RRTMG-IFS") THEN
    NSWB_MNH = 14  ! RRTM bands
  ELSE
    NSWB_MNH = 32  ! ECCKD bands - WARNING: change must be made on this number if gas model bands are changed
  END IF
    NLWB_MNH = 16
ELSE
    NSWB_MNH = NSWB_OLD
...
CALL READ_FIELD(KMI,TPINIFILE,IIU,IJU,IKU,                                    &
                ...
                XWUMF, XWVMF,                                                 &
                XPHC,XPHR, XLSUM,XLSVM,XLSWM,XLSTHM,XLSRVM,XLSZWSM,XLSSVM,    &
                ...
                XLSPHI, XBMAP, XFMASE, XFMAWC, XFMWINDU, XFMWINDV, XFMWINDW, XFMHWS, KMI)
...
!
!*       13.0   INITIALIZE ACLIB (AEROSOL CHEMISTRY LIBRARY)
!               ----------------------------
!
! ACLIB Chemistry structures (config and variables parameters)
IF(LUSECHEM) CALL INIT_CHEM_ACLIB(ILUOUT,KMI)
! ACLIB Aerosols structures (config and variables parameters)
IF(LDUST .OR. LSALT .OR. LORILAM) CALL INIT_AER_ACLIB(ILUOUT, KMI)
! ACLIB-MesoNH interface
IF(LDUST .OR. LSALT .OR. LORILAM .OR. LUSECHEM) CALL INI_MNH_ACLIB()
! Init of aerosol prognostic variables
IF(LDUST .OR. LSALT .OR. LORILAM ) THEN
  CALL INIT_AER_MODEL(YPAER_ACLIB, IIB*IJB, IIE*IJE, IIU*IJU, IKU, ILUOUT, YAEROUT_ACLIB)
END IF
! INIT_CHEM_MODEL (init of chemistry of other models than mesonh) is done in modeln
!-------------------------------------------------------------------------------
!
!*       13.1    INITIALIZE THE ATMOSPHERIC ELECTRICITY
!               --------------------------------------
!
IF (CELEC /= 'NONE') THEN
...
IF ((KMI==1).AND.(.NOT. LSTEADYLS)) THEN
  ...
  CALL INI_CPL(NFINAL_TIMESTEP,XTSTEP,LSTEADYLS,                              &
               CGETTKET,                                                      &
               ...
               NSIZELBXTKE_ll,NSIZELBYTKE_ll,                                 &
               NSIZELBXR_ll,NSIZELBYR_ll,NSIZELBXSV_ll,NSIZELBYSV_ll,         &
               XLSUM,XLSVM,XLSWM,XLSTHM,XLSRVM,XLSZWSM,XLSSVM,XDRYMASST,      &
               XLBXUM,XLBXVM,XLBXWM,XLBXTHM,XLBXTKEM,XLBXRM,XLBXSVM,          &
               XLBYUM,XLBYVM,XLBYWM,XLBYTHM,XLBYTKEM,XLBYRM,XLBYSVM,          &
               XLSUS,XLSVS,XLSWS,XLSTHS,XLSRVS,XLSZWSS,XLSSVS,XDRYMASSS,      &
...
IF ( KMI > 1) THEN
  ...
  CALL INI_SPAWN_LS_n(NDAD(KMI),XTSTEP,KMI,                                 &
       ...
       LSLEVE,XLEN1,XLEN2,                                   &
       DPTR_XLSUM,DPTR_XLSVM,DPTR_XLSWM,DPTR_XLSTHM,DPTR_XLSRVM,DPTR_XLSZWSM,DPTR_XLSSVM, &
       DPTR_XLSUS,DPTR_XLSVS,DPTR_XLSWS,DPTR_XLSTHS,DPTR_XLSRVS,DPTR_XLSZWSS,DPTR_XLSSVS, &
...
CALL INI_DYNAMICS(XLON,XLAT,XRHODJ,XTHVREF,XMAP,XZZ,XDXHAT,XDYHAT,            &
             ...
             LHORELAX_SVC2R2,LHORELAX_SVLIMA,LHORELAX_SVELEC,LHORELAX_SVLG,   &
...
IF (LCOUPLES.AND.(KMI>1))THEN
  CSURF ="NONE"
ELSE
  CALL IO_Field_read(TPINIFILE,'SURF',CSURF,IRESP)
  IF ( IRESP /= 0 ) CSURF = 'EXTE' ! Set a default value (field not present in old MNH files < 4.6)
END IF
!
!
IF (CSURF=='EXTE' .AND. (CPROGRAM=='MESONH' .OR. CPROGRAM=='DIAG  ')) THEN
  ...
  IF  ( LEN_TRIM(CINIFILEPGD) > 0 ) THEN
    CALL IO_File_add2list( TINIFILEPGD, TRIM(CINIFILEPGD), 'PGD', 'READ' )
...
IF (LCHEMDIAG)  THEN
  ...
  IF(LUSECHEMTABLE) THEN 
    NIND_SPEC(:) = YPCHEM_ACLIB%KITRAPROD(:)
  ELSE
    IF(ALLOCATED(YPCHEM_ACLIB%KITRAPROD)) DEALLOCATE(YPCHEM_ACLIB%KITRAPROD) ! Allocated in init_param_chem_aclib
    ALLOCATE(YPCHEM_ACLIB%KITRAPROD(NEQ))
    YPCHEM_ACLIB%KITRAPROD(:) = NIND_SPEC(:)
  END IF
