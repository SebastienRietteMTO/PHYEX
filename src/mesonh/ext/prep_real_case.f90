...
!     ######################
!
!!****  *PREP_REAL_CASE* - program to write an initial MesoNH file from real case
!!                         situation.
!!
!!    PURPOSE
!!    -------
...
!!    filled by some fields of a real situation.
...
!!        the descriptive (.des) file (by calling DEFAULT_DESCRIPTORS_n); lateral boundary conditions
!!        are open.
!!
...
!!        namelist file (routines READ_GENERAL in
...
!!    --------
!!
!!      Routine DEFAULT_DESCRIPTORS_n : to set default values for variables which can be
!!                              contained in the descriptive (.des) file.
...
!!                              initialized
!!      Routine WRITE_DESCRIPTIVE_FILE_n : to write a descriptive (.des) file.
!!      Routine WRITE_MODELSTATE_n       : to write the data (model state).
!!      Routine IO_File_close : to close files
!!
!!      Module MODE_GRIDPROJ  : contains conformal projection routines
!!
!!      Module MODI_DEFAULT_DESCRIPTORS_n  : interface module for routine DEFAULT_DESCRIPTORS_n
...
!!      Module MODI_WRITE_DESCRIPTIVE_FILE_n : interface module for routine WRITE_DESCRIPTIVE_FILE_n
!!      Module MODI_WRITE_MODELSTATE_n : interface module for routine WRITE_MODELSTATE_n
!!      
!!
!!    IMPLICIT ARGUMENTS
!!    ------------------
...
!!         CINIFILE: name of the file which will be used for the MESO-NH run.
...
!!                  Apr   17, 1996  (J. Stein ) change the DEFAULT_DESCRIPTORS_n CALL
...
USE MODD_CONF_n
USE MODD_CST
USE MODD_CST_ACLIB, ONLY: YDCST_ACLIB
USE MODD_DIM_n
!UPG*PT
USE MODD_CH_AEROSOL
...
USE MODD_NSV
USE MODD_PARAMETERS
USE MODD_PARAM_LIMA,       ONLY: PARAM_LIMA_INIT, NMOD_CCN, NMOD_IFN, CIFN_SPECIES, CCCN_MODES
USE MODD_PARAM_n
USE MODD_PREP_REAL
USE MODD_REF_n
!UPG*PT
...
USE MODD_HALO_D,           ONLY: INIT_HALO_D
#endif
...
USE MODI_DEFAULT_DESCRIPTORS_n
...
USE MODI_VERSION
USE MODI_VER_THERMO
USE MODI_WRITE_DESCRIPTIVE_FILE_n
USE MODI_WRITE_MODELSTATE_n
USE M_INIT_CST_ACLIB, ONLY: INIT_CST_ACLIB
!
...
INTEGER                           :: IRESP    ! return code of I/O routines
...
!-------------------------------------------------------------------------------
!
CALL IO_File_add2list( TINIFILE, CINIFILE, 'MNH', 'WRITE' )
...
IF (LORILAM) THEN
  ...
  ZRHODREFMAX = MAX_ll( XRHODREF(:,:,:), IINFO_ll)

  CALL CH_AER_EQM_INIT_n(YDCST_ACLIB, CCONF, CPROGRAM, XSVT(:,:,:,NSV_CHEMBEG:NSV_CHEMEND),&
                         XSVT(:,:,:,NSV_AERBEG:NSV_AEREND),&
                         XM3D,XRHOP3D,XSIG3D,&
                         XRG3D,XN3D, XRHODREF, XCTOTA3D,ZRHODREFMAX,&
                         SIZE(XSVT,1), SIZE(XSVT,2), SIZE(XSVT,3), NSV_CHEM, NSV_AER )
END IF
!
...
IF (CCLOUD == 'LIMA' .AND. ((LORILAM).OR.(LDUST).OR.(LSALT))) THEN
    ...
    DO JI=1,NMOD_CCN
      XLBXSVM(:,:,:,NSV_LIMA_CCN_FREE+JI-1)   = ZLBXSVM(:,:,:,NSV_LIMA_CCN_FREE+JI-1)
      XLBYSVM(:,:,:,NSV_LIMA_CCN_FREE+JI-1)   = ZLBYSVM(:,:,:,NSV_LIMA_CCN_FREE+JI-1)
    END DO

    DO JI=1,NMOD_IFN
      XLBXSVM(:,:,:,NSV_LIMA_IFN_FREE+JI-1)   = ZLBXSVM(:,:,:,NSV_LIMA_IFN_FREE+JI-1)
      XLBYSVM(:,:,:,NSV_LIMA_IFN_FREE+JI-1)   = ZLBYSVM(:,:,:,NSV_LIMA_IFN_FREE+JI-1)
    END DO
...
!-------------------------------------------------------------------------------
!
!*      17.    WRITING OF THE MESO-NH FILE
!              ---------------------------
!
ZTIME1 = ZTIME2
!
...
END IF
!
CALL WRITE_DESCRIPTIVE_FILE_n( 1, TINIFILE )
CALL IO_Header_write(TINIFILE,HDAD_NAME=YDAD_NAME)
CALL WRITE_MODELSTATE_n(TINIFILE,YDAD_NAME)
! 
CALL SECOND_MNH(ZTIME2)
